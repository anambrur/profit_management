...
// Common function for order processing
async function processOrders() {
  try {
    const jobStartTime = new Date();
    sendNotification(
      'info',
      `Order processing started at ${jobStartTime.toISOString()} - Processing all stores`
    );
    ...
    for (const [index, store] of stores.entries()) {
      ...
      console.log(
        `\nStarting store for order processing ${index + 1}/${stores.length}: ${store.storeId} (${store.storeName})`
      );
    ...
    // Final summary
    const totalTime = Math.round((Date.now() - jobStartTime.getTime()) / 60000);
    const successRate = Math.round((processedStores / stores.length) * 100);

    console.log(
      `\nProcessing completed in ${totalTime}m\n` +
    ...
    return {
      ...
      ordersCreated: grandTotalCreated,
      ordersSkipped: grandTotalSkipped,
      ordersFailed: grandTotalFailed,
      stockAlerts: grandTotalStockAlerts,
    };
  } catch (error) {
    const errorMessage =
      error instanceof Error ? error.message : 'Unknown error';
    ...
    sendNotification(
      'error',
      `‼️ Order processing failed catastrophically\n` + `Error: ${errorMessage}`
    );
...
// Common function for product processing
async function processProducts() {
  try {
    ...
    sendNotification(
      'info',
      `Product processing started at ${startTime.toISOString()} - Processing all stores`
    );
    ...
    for (const [index, store] of stores.entries()) {
      ...
      console.log(
        `\nStarting store for product processing ${index + 1}/${stores.length}: ${store.storeId}`
      );
    ...
    return {
      ...
      storesProcessed: processedStores,
      storesFailed: failedStores,
      itemsProcessed: grandTotalProcessed,
    ...
    sendNotification(
      'error',
      `‼️ Product processing failed catastrophically\n` +
        `Error: ${errorMessage}`
    );
...
export { OrderCornJob, processOrders, processProducts, ProductCornJob };
